...
# This suite lives in Evaluation/ and imports core modules from the sibling SRC/folder.
...
def test_prexdr_injectable_keeps_bpalm():
    # PreXDR by injectable resistance only: fluoroquinolones remain usable, so BPaLM
...
def test_regimen_mode_tracks_baseline():
    ...
    # regimen, so it should recover accuracy the outcome-ranked recommender loses to
